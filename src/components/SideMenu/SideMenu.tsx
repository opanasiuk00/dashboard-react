...
import { CustomersIcon, DashboardIcon, HelpIcon, ProductIcon, PromoteIcon } from '../Icons';
import { useLocation, useMatch } from 'react-router-dom';

export const SideMenu = ({ isActiveMobile, setIsActiveMobile, className = '' }: SideMenuProps): JSX.Element => {
	...
	return (
		<div className={`${styles.menu} ${isActiveMobile ? styles.active : ''} ${className}`}>
			<h4>
				<svg width="37" height="37" viewBox="0 0 37 37" fill="black" xmlns="http://www.w3.org/2000/svg">
			...
			<nav className={styles.nav}>
				<MenuLink icon={<DashboardIcon />} href='/' onClick={closeMenu}>
					Dashboard
				</MenuLink>
				<MenuLink icon={<ProductIcon />} href='/Product' onClick={closeMenu}>
					Product
				</MenuLink>
				<MenuLink icon={<CustomersIcon />} href='/Customers' className='active' onClick={closeMenu}>
					Customers
				</MenuLink>
				<MenuLink icon={<PromoteIcon />} href='/Promote' onClick={closeMenu}>
					Promote
				</MenuLink>
				<MenuLink icon={<HelpIcon />} href='/Help' onClick={closeMenu}>
					Help
				</MenuLink>
			</nav>
			<User className={styles.user} image='asda' name='Evano' position='Project Manager' />
		</div>
	)
};
